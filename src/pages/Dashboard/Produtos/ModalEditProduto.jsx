...
const ModalEditProduto = () => {
  ...
  useEffect(() => {
    if (data && id) {
      const editedProduto = data.find((produto) => produto.id == id);
      editedProduto && setEditingProduto(editedProduto);
      !editedProduto && close()
    }
  }, [data, id]);
  ...
  const onSubmit = async (e) => {
    ...
    produtoFormData.append('importado', +inputIsImportado?.current?.checked)

    // for (const [key, value] of produtoFormData.entries()) {
    //   console.log(key, value);
    // }
    // console.log('form',produtoFormData.getAll())
